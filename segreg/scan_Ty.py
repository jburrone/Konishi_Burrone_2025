...
def parse_args():
    ...
    parser.add_argument("--angle", default=0, type=int, help="the center value of angle Y search")
...
if __name__ == "__main__":
    ...
    for i in range(len(sxlist)):
        ...
        # for i, (augmented_ref_img, augmented_ref_mask) in enumerate(
        #     zip(augmented_ref_imgs, augmented_ref_masks)
        # ):
        #     ref_img = transform3d.pad_image(augmented_ref_img)
        #     ref_seg = transform3d.pad_image(augmented_ref_mask)
        if ref_seg.ndim == 3:
            ref_img = ref_img[ref_img.shape[0] // 2]
        ...
        mov_img_and_seg = mov_img_crop3d_reg, mov_seg_crop3d_reg
        register.scanTy(
